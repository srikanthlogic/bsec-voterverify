...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        int i = AlErrorCode.ERR_NOT_SUPPORTED;
        if (!canScroll) {
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, AlErrorCode.ERR_NOT_SUPPORTED);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(AlErrorCode.ERR_NOT_SUPPORTED)) {
                return false;
            }
            return true;
            ...
            if (keyCode != 19) {
                if (keyCode != 20) {
                    ...
                    return arrowScroll(AlErrorCode.ERR_NOT_SUPPORTED);
                } else {
                    return fullScroll(AlErrorCode.ERR_NOT_SUPPORTED);
                }
            } else if (!event.isAltPressed()) {
    ...
    protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = AlErrorCode.ERR_NOT_SUPPORTED;
        } else if (direction == 1) {
            direction = 33;
        }
