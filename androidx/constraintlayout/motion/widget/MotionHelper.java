...
public class MotionHelper extends ConstraintHelper implements Animatable, MotionLayout.TransitionListener {
    ...
    public void init(AttributeSet attrs) {
        super.init(attrs);
        if (attrs != null) {
            TypedArray a2 = getContext().obtainStyledAttributes(attrs, R.styleable.MotionHelper);
            int N = a2.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a2.getIndex(i);
                if (attr == R.styleable.MotionHelper_onShow) {
                    this.mUseOnShow = a2.getBoolean(attr, this.mUseOnShow);
                } else if (attr == R.styleable.MotionHelper_onHide) {
                    this.mUseOnHide = a2.getBoolean(attr, this.mUseOnHide);
                }
            }
            a2.recycle();
        }
