...
final class PhoneNumberMatcher implements Iterator<PhoneNumberMatch> {
    ...
    public boolean checkNumberGroupingIsValid(Phonenumber.PhoneNumber number, CharSequence candidate, PhoneNumberUtil util, NumberGroupingChecker checker) {
        ...
        Phonemetadata.PhoneMetadata alternateFormats = util.getMetadataSource().getAlternateFormatsForCountry(number.getCountryCode());
        String nationalSignificantNumber = util.getNationalSignificantNumber(number);
        if (alternateFormats != null) {
            for (Phonemetadata.NumberFormat alternateFormat : alternateFormats.numberFormats()) {
                if (alternateFormat.leadingDigitsPatternSize() <= 0 || this.regexCache.getPatternForRegex(alternateFormat.getLeadingDigitsPattern(0)).matcher(nationalSignificantNumber).lookingAt()) {
