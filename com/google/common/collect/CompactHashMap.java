...
public class CompactHashMap<K, V> extends AbstractMap<K, V> implements Serializable {
    ...
    public Map<K, V> convertToHashFloodingResistantImplementation() {
        ...
        while (i >= 0) {
            newDelegate.put((K) this.keys[i], (V) this.values[i]);
            i = getSuccessor(i);
        }
    ...
    /* JADX INFO: Multiple debug info for r1v4 int: [D('entry' int), D('next' int)] */
    ...
    public V put(@NullableDecl K key, @NullableDecl V value) {
        ...
        if (entry != 0) {
            ...
            while (true) {
                ...
                int next2 = entries[entryIndex];
                if (CompactHashing.getHashPrefix(next2, mask) != hashPrefix2 || !Objects.equal(key, keys[entryIndex])) {
                    ...
                    if (next3 != 0) {
                        ...
                        if (newSize > mask) {
                            ...
                            entries[entryIndex] = CompactHashing.maskCombine(next2, newEntryIndex + 1, mask);
                            hashPrefix = mask;
                        }
                    }
                } else {
    ...
    /* JADX INFO: Multiple debug info for r1v0 int: [D('size' int), D('newDelegate' java.util.Map<K, V>)] */
