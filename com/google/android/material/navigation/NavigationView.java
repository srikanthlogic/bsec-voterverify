...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    public NavigationView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TintTypedArray a2 = ThemeEnforcement.obtainTintedStyledAttributes(context, attrs, R.styleable.NavigationView, defStyleAttr, R.style.Widget_Design_NavigationView, new int[0]);
        ViewCompat.setBackground(this, a2.getDrawable(R.styleable.NavigationView_android_background));
        if (a2.hasValue(R.styleable.NavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) a2.getDimensionPixelSize(R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, a2.getBoolean(R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.maxWidth = a2.getDimensionPixelSize(R.styleable.NavigationView_android_maxWidth, 0);
        if (a2.hasValue(R.styleable.NavigationView_itemIconTint)) {
            itemIconTint = a2.getColorStateList(R.styleable.NavigationView_itemIconTint);
        } else {
            itemIconTint = createDefaultColorStateList(16842808);
        }
        boolean textAppearanceSet = false;
        int textAppearance = 0;
        if (a2.hasValue(R.styleable.NavigationView_itemTextAppearance)) {
            textAppearance = a2.getResourceId(R.styleable.NavigationView_itemTextAppearance, 0);
            textAppearanceSet = true;
        }
        ColorStateList itemTextColor = a2.hasValue(R.styleable.NavigationView_itemTextColor) ? a2.getColorStateList(R.styleable.NavigationView_itemTextColor) : null;
        if (!textAppearanceSet && itemTextColor == null) {
            itemTextColor = createDefaultColorStateList(16842806);
        }
        Drawable itemBackground = a2.getDrawable(R.styleable.NavigationView_itemBackground);
        if (a2.hasValue(R.styleable.NavigationView_itemHorizontalPadding)) {
            this.presenter.setItemHorizontalPadding(a2.getDimensionPixelSize(R.styleable.NavigationView_itemHorizontalPadding, 0));
        }
        int itemIconPadding = a2.getDimensionPixelSize(R.styleable.NavigationView_itemIconPadding, 0);
        ...
        if (a2.hasValue(R.styleable.NavigationView_menu)) {
            inflateMenu(a2.getResourceId(R.styleable.NavigationView_menu, 0));
        }
        if (a2.hasValue(R.styleable.NavigationView_headerLayout)) {
            inflateHeaderView(a2.getResourceId(R.styleable.NavigationView_headerLayout, 0));
        }
        a2.recycle();
    }
