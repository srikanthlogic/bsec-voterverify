...
public class RequestLoggingListener implements RequestListener {
    ...
    public synchronized void onProducerFinishWithFailure(String requestId, String producerName, Throwable throwable, @Nullable Map<String, String> extraMap) {
        if (FLog.isLoggable(5)) {
            Pair<String, String> mapKey = Pair.create(requestId, producerName);
            long currentTime = getTime();
            FLog.w(TAG, throwable, "time %d: onProducerFinishWithFailure: {requestId: %s, stage: %s, elapsedTime: %d ms, extraMap: %s, throwable: %s}", Long.valueOf(currentTime), requestId, producerName, Long.valueOf(getElapsedTime(this.mProducerStartTimeMap.remove(mapKey), currentTime)), extraMap, throwable.toString());
        }
    }
    ...
    public synchronized void onUltimateProducerReached(String requestId, String producerName, boolean successful) {
        if (FLog.isLoggable(2)) {
            Pair<String, String> mapKey = Pair.create(requestId, producerName);
            long currentTime = getTime();
            FLog.v(TAG, "time %d: onUltimateProducerReached: {requestId: %s, producer: %s, elapsedTime: %d ms, success: %b}", Long.valueOf(currentTime), requestId, producerName, Long.valueOf(getElapsedTime(this.mProducerStartTimeMap.remove(mapKey), currentTime)), Boolean.valueOf(successful));
        }
    }
    ...
    public synchronized void onRequestSuccess(ImageRequest request, String requestId, boolean isPrefetch) {
        if (FLog.isLoggable(2)) {
            long currentTime = getTime();
            FLog.v(TAG, "time %d: onRequestSuccess: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(currentTime), requestId, Long.valueOf(getElapsedTime(this.mRequestStartTimeMap.remove(requestId), currentTime)));
        }
    }
    ...
    public synchronized void onRequestCancellation(String requestId) {
        if (FLog.isLoggable(2)) {
            long currentTime = getTime();
            FLog.v(TAG, "time %d: onRequestCancellation: {requestId: %s, elapsedTime: %d ms}", Long.valueOf(currentTime), requestId, Long.valueOf(getElapsedTime(this.mRequestStartTimeMap.remove(requestId), currentTime)));
        }
    }
