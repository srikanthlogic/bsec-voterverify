...
public final class AwaitAll<T> {
    ...
    public final Object await(Continuation<? super List<? extends T>> continuation) {
        ...
        while (i$iv < length) {
            ...
            deferred.start();
            AwaitAllNode $receiver = new AwaitAllNode(this, cont, deferred);
            $receiver.setHandle(deferred.invokeOnCompletion($receiver));
            awaitAllNodeArr[i$iv] = $receiver;
            i$iv++;
            $i$f$suspendCancellableCoroutine = $i$f$suspendCancellableCoroutine;
            uCont$iv = uCont$iv;
        }
        DisposeHandlersOnCancel disposer = new DisposeHandlersOnCancel(this, awaitAllNodeArr);
        for (AwaitAllNode awaitAllNode : awaitAllNodeArr) {
    ...
    public final class DisposeHandlersOnCancel extends CancelHandler {
        ...
        public DisposeHandlersOnCancel(AwaitAll $outer, AwaitAll<T>.AwaitAllNode[] awaitAllNodeArr) {
    ...
    public final class AwaitAllNode extends JobNode<Job> {
        ...
        /* JADX WARN: Multi-variable type inference failed */
        public AwaitAllNode(AwaitAll $outer, CancellableContinuation<? super List<? extends T>> cancellableContinuation, Job job) {
            super(job);
        ...
        public void invoke2(Throwable cause) {
            ...
            if (AwaitAll.notCompletedCount$FU.decrementAndGet(this.this$0) == 0) {
                ...
                cancellableContinuation.resumeWith(Result.m13constructorimpl((List) destination$iv$iv));
            }
        }
    }
}
